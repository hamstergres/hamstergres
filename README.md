...
PostgreSQL deadlock or serialization failures. Operators who accept partial
cross-Burrow commit risk may set
...
The I/O-bound Proxy defaults to one Go scheduler execution thread to reduce CPU
wakeups and context switching. Set `runtime.max_procs` in the configuration, or
set `GOMAXPROCS` when `runtime.max_procs` is omitted, after benchmarking a
deployment that benefits from more Proxy CPU parallelism.
